...
def listar_avarias(loja: str | None = None, data_inicio=None, data_fim=None):
    ...
    defeito), já com dados da loja/documento/item — é a fonte de dados da
    tela de Qualidade. Como lê direto da mesma tabela que a tratativa
    grava, qualquer AVARIA lançada na tela de Devoluções (setor Expedição)
    aparece aqui automaticamente, sem nenhuma sincronização extra."""
    with _conn() as conn:
        with conn.cursor() as cur:
...
def indicadores_avarias_por_loja():
    """Total de peças em avaria e nº de devoluções distintas com avaria,
    agrupado por loja — usado nos indicadores da tela de Qualidade."""
    with _conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT
                    d.loja,
                    SUM(dt.quantidade) AS total_avaria,
